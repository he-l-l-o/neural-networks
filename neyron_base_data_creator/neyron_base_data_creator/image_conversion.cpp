...
void get_color_array(short color_of_image_pixels[8][8][3], int type_lien, sf::Image load_image, sf::Event event, int location_marker_x, int location_marker_y) {
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			...
			type_lien = event.key.code - 26;

			std::cout << color_of_image_pixels[j][i][0] << " ";
			std::cout << color_of_image_pixels[j][i][1] << " ";
			std::cout << color_of_image_pixels[j][i][2] << " ";
			std::cout << type_lien << std::endl;
